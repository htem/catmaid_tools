...
echo "#id,source,target,location" > connectors.csv
psql catmaid catmaid_user -t -A -F "," -c "
SELECT ct.id, nt.source, nt.target, ct.location FROM
((SELECT connector_id as source_connector_id, skeleton_id as source FROM treenode_connector
  WHERE project_id = 9 AND relation_id = (SELECT id FROM relation WHERE project_id = 9 and relation_name = 'presynaptic_to')) AS prt
FULL OUTER JOIN (SELECT connector_id as target_connector_id, skeleton_id as target FROM treenode_connector
  WHERE project_id = 9 AND relation_id = (SELECT id FROM relation WHERE project_id = 9 and relation_name = 'postsynaptic_to')) AS pot
    ON (prt.source_connector_id = pot.target_connector_id)) AS nt
JOIN (SELECT id, location FROM connector WHERE project_id = 9) ct
