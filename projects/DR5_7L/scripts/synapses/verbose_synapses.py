...
def verbose_synapses(a, d, s):
    na = s.get_neuron(a)
    nd = s.get_neuron(d)
    r = catmaid.algorithms.population.synapses.skeleton_overlap_v_verbose(
        na, nd, 4000., 20000.)
    fn = '../../results/synapses/pair_values/pair_{}_{}'.format(a, d)
    if not os.path.exists(os.path.dirname(fn)):
        os.makedirs(os.path.dirname(fn))
    # with open(fn, 'w') as f:
    #     for l in r:
    #         f.write(','.join(map(str, l)) + '\n')
    scipy.io.savemat(fn, {'pair_{}_{}'.format(a, d): numpy.array(r)})


n_jobs = -1
output_file = None
function = verbose_synapses
